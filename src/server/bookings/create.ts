...
const schema = z.object({
  ...
  deposit_amount: z
    .preprocess((v) => (v === '' || v === undefined ? 0 : v), z.coerce.number().nonnegative())
    .default(0),
...
export async function createBooking(formData: FormData): Promise<CreateBookingResult> {
  ...
  // Leere Strings zu undefined für optionale Felder
  for (const k of [
    'channel_id',
    'external_reference',
  ...
  if (bookingErr) {
    // DB-Constraint hat zugeschlagen → trotzdem hilfreiche Meldung
  ...
  // Workflow-Aufgaben instanziieren (Einzug + Auszug)
