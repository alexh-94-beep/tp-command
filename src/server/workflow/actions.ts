...
export async function completeTask(taskId: string): Promise<Result> {
  ...
  await revalidateForTask(t.booking_id as string);
  return { ok: true };
}
...
export async function addManualTask(formData: FormData): Promise<Result> {
  ...
  // Position = letzte + 1 für die kind-Gruppe
...
export async function deleteManualTask(taskId: string): Promise<Result> {
  ...
  if (t.template_task_id) {
    return { ok: false, error: 'Nur manuelle Aufgaben können gelöscht werden. Setze stattdessen den Status auf „N/A".' };
  }
...
export async function regenerateBookingTasks(bookingId: string): Promise<Result & { created?: number }> {
  await requireRole(['admin', 'office']);
  const supabase = createSupabaseServerClient();
