...
export default function NewBookingForm({
  ...
  /** Wird nicht mehr für Pre-Loading genutzt – Wohnungen werden dynamisch geladen */
  apartments?: never;
  channels: ChannelOpt[];
  ...
  // Hole verfügbare Wohnungen, sobald Einzug gesetzt ist.
  useEffect(() => {
    if (!startDate) {
      setApartments([]);
      return;
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        setApartments(r);
        // Wenn die aktuell gewählte Wohnung nicht mehr verfügbar ist, deselectieren
        if (apartmentId) {
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>
                  Auszug{' '}
                  <span className="text-slate-400">(leer = Langzeit, unbefristet)</span>
                </label>
                <DateInput
                  name="end_date"
                  value={endDate}
          ...
          <CardBody className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                {fe.first_name && <p className="mt-1 text-xs text-red-600">{fe.first_name[0]}</p>}
              </div>
              <div>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardBody className="space-y-4">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              ...
              <div>
                <label className={labelCls}>
                  Mietzins (CHF){selectedApartment && (
                    <span className="ml-2 text-xs font-normal text-slate-500">
                      Standard:{' '}
