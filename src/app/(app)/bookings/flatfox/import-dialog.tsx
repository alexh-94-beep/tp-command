...
export function ImportDialog({ row, onClose, onSuccess }: ImportDialogProps) {
  ...
  // Wohnungen laden wenn manuelle Zuordnung noetig (async IIFE-Pattern)
  useEffect(() => {
    if (!needsManualAssign) return;
    let cancelled = false;
    (async () => {
      const r = await listApartmentsForFlatfoxAssign();
      if (cancelled) return;
              ...
              <div className="mt-3 space-y-2">
                ...
                {apartments && filteredApts.length === 0 && (
                  <div className="text-xs text-slate-500">
                    Keine Treffer für „{apartmentFilter}&ldquo;.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setContractStatus(e.target.value as 'draft' | 'sent' | 'signed')
                }
              >
