...
export default function FlatfoxImportList() {
  ...
  useEffect(() => {
    refresh(onlyWithForm);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onlyWithForm]);

  function refresh(withForm = onlyWithForm) {
    setError(null);
    startTransition(async () => {
      const r = await listFlatfoxApplications({ onlyWithForm: withForm });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {rows && (
          <span className="text-xs text-slate-500">
            {rows.length} Anmeldung(en) · {rows.filter((r) => !r.is_imported).length} noch nicht übernommen
          </span>
        )}
      </div>
      ...
      {rows && rows.length === 0 && (
        <Card>
          <CardBody className="text-sm text-slate-500">
            Keine Anmeldungen vorhanden – sobald jemand auf Flatfox eine Anmeldung schickt, erscheint sie hier.
          </CardBody>
        </Card>
      )}
                ...
                <tr key={r.pk} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    {r.apartment_in_db_id ? (
                      <Link
                        href={`/apartments/${r.apartment_in_db_id}`}
