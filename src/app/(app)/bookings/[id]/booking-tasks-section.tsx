      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-2">
            ...
            {task.is_optional && (
              <span className="text-[10px] tracking-wide text-slate-400 uppercase">optional</span>
            )}
            {task.is_conditional && (
              <span className="text-[10px] tracking-wide text-amber-600 uppercase">bedingt</span>
            )}
            {task.status !== 'open' && (
              ...
              {showNotes ? (
                <div className="flex flex-col gap-2">
                  <textarea
                    ...
                    rows={2}
                    className="w-full rounded-md border border-slate-300 px-2 py-1 text-xs focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none"
                    placeholder="Notiz …"
                  />
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute top-full right-0 z-30 mt-1 w-48 rounded-lg border border-slate-200 bg-white py-1 text-sm shadow-lg">
                {!showNotes && (
                  <button
                    onClick={() => {
                      setMenuOpen(false);
  ...
  return (
    <div className="flex items-center gap-1">
      <input
        ...
        className="rounded-md border border-slate-300 px-2 py-1 text-xs focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none"
      />
      <input
        type="date"
        value={dueDate}
...
function RegenerateButton({ bookingId, hasAny }: { bookingId: string; hasAny: boolean }) {
  const [pending, startTransition] = useTransition();
  return (
