...
export default function DamageReportButton() {
  ...
  const weekStart = mondayOfWeekIso(today);
  ...
  function setPreset(preset: 'today' | 'week' | 'last-week' | 'month') {
    ...
    if (preset === 'today') {
      ...
      const ws = mondayOfWeekIso(t);
      setFrom(ws);
      setTo(addDaysIso(ws, 6));
    } else if (preset === 'last-week') {
      const ws = mondayOfWeekIso(t);
      const lws = addDaysIso(ws, -7);
      setFrom(lws);
      setTo(addDaysIso(lws, 6));
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full right-0 z-50 mt-2 w-[340px] rounded-xl border border-slate-200 bg-white p-4 shadow-lg">
          ...
          <div className="mt-3 flex flex-wrap gap-1 text-xs">
            {[
              ...
              <button
                ...
                type="button"
                onClick={() =>
                  setPreset(p.v as 'today' | 'week' | 'last-week' | 'month')
                }
