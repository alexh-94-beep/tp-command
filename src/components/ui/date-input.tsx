...
export function DateInput({ name, value, onChange, required, className, id }: DateInputProps) {
  ...
  // React 19 "derived state" pattern: synchronisiere Textanzeige mit dem
  // externen value waehrend des Render, nicht in einem Effect.
  if (prevValue !== value) {
    setPrevValue(value);
  ...
  return (
    <div ref={wrapperRef} className={cn('relative', className)}>
      <input
        ...
        className={cn(
          'block w-full rounded-md border bg-white px-3 py-2 pr-10 text-sm shadow-sm focus:ring-1 focus:outline-none',
          error
      ...
      <button
        ...
        className="absolute top-1/2 right-2 -translate-y-1/2 rounded p-1 text-slate-500 hover:bg-slate-100 hover:text-slate-700"
      >
